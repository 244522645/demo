						...
						<label class="weui-cell weui-check__label img-mask" for="s${entity.id}">  
                            ...
                            <img  dt="${entity.imgUrl}_y1080.jpg" src="${entity.imgUrl}_348x232.jpg">
                               <div class="maskcheckbox">
                                        <input  img="${entity.imgUrl}_y1080.jpg"  local="${entity.localName}" addr="${entity.title}" dt='<fmt:formatDate value="${entity.shootingTime }" pattern="yyyy/MM/dd"/>' type="radio" class="weui-check" name="checkbox1" id="s${entity.id}" value="${entity.id}" <c:if test="${status.index eq 0}"> checked="checked"</c:if>>
    ...
    <div class="page__bd">
            ...
            <div class="weui-cell weui-cell_select weui-cell_select-after">
				...
				<c:if test="${not empty shop}">
 					<div class="weui-select">
						${shop.name}
						<input  id="input_select_shop" type="hidden" name="select2" value="${shop.name}"/>
					</div>
				</c:if>
				<c:if test="${not empty shopList}">
					<div class="weui-select">
						官方旗舰店
						<input  id="input_select_shop" type="hidden" name="select2" value="官方旗舰店"/>
					</div>
        ...
        <div class="weui-cells weui-cells_form">
            ...
            <div class="weui-cell weui-cell_vcod ">
                ...
                <div class="weui-cell__bd">
                    <input id="input_phone" class="weui-input" type="tel"  pattern="/^1[34578]\d{9}$/" placeholder="请输入正确手机号">
                </div>
            </div>
        ...
        <div class="weui-cells weui-cells_form">
            <div class="weui-cell">
                <div class="weui-cell__bd">
                    <textarea id="input_addr" class="weui-textarea" placeholder="请输入详细地址" rows="2"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="weui-cells weui-cells_form">
            <div class="weui-cell">
                <div class="weui-cell__bd">
                    <textarea id="input_note" class="weui-textarea" placeholder="请输入祝福语和落款" rows="2"></textarea>
                    <div class="weui-textarea-counter">少于40字</div>
                </div>
            </div>
        </div>
...
$(function(){
	...
	//	支付回到 修改状态
	function showOrder(){
		$.post(ctx+"/wechat/cake/setStatus?id="+cakeOrderId ,function(result){
			console.log(result);
	...
	function tooltipsShow(){
        ...
        $('#showTooltips').on('click', function(){
			...
			var reg = /^1[34578]\d{9}$/;     
        	var r = input_phone.match(reg);    
			  if(r==null)  {  
				
				$tooltips.html('请输入正确的手机号');
			...
			$.post(ctx+"/wechat/cake/put", putjson,function(result){
				if(result.state==1){
					$("#weui-toast").html('<br><br>提交成功<br>请支付');
					var orderInfoHtml="";
					...
					cakeOrderId=result.t.id;
					wxPayManager.init(input_photoInfo,input_photoInfo,19.00,'520A99'+result.t.number);
					wxPayManager.loadPay(showOrder,cancelOrder);
...
$(function(){
  ...
  <script type="text/javascript" src="https://res.wx.qq.com/open/js/jweixin-1.0.0.js"></script>
    ...
    <script src="${ctx}/static/weui/test.js?v=1.1"></script>
